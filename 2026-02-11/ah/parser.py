...
class Parser:
    ...
    def start(self):
        ...
        products = list(self.mongo[MONGO_COLLECTION_RESPONSE].find().limit(250))
    ...
    def parse_item(self, response):
        ...
        price_xpath = "//div[contains(@class,'product-hero-title_unitInfo')]//span[last()]"
        ...
        promotion_description_xpath = "//div[@data-key='pdp-shield-discount']/@aria-label"
        ...
        for script in sel.xpath(image_scripts_xpath).getall():
            ...
            if "self.__next_f.push" in script and "product-detail-page" in script:
                ...
                for url in matches:
                    ...
                    if product_id not in best_images:
                        best_images[product_id] = url
                    else:
                        if "800x800" in url:
                            best_images[product_id] = url
                        elif "400x400" in url:
        ...
        raw_text = sel.xpath(f"normalize-space({price_xpath})").get()
        price_per_unit = " ".join(raw_text.split())
        ...
        for product_data in product_details:
            ...
            image_url1 = json_data.get("image")
            logging.info(f"image_url1: {image_url1}")
